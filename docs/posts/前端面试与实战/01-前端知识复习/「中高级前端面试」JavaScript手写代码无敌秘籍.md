---
title: "「中高级前端面试」JavaScript手写代码无敌秘籍"
date: 2026-08-11
categories:
  - "前端面试与实战"
tags:
  ...
  - "OneNote"
  - "前端知识复习"
description: "2019 09 29阅读 1970 手写路径导航 实现一个new操作符 实现一个JSON.stringify 实现一个JSON.parse 实现一个call或 apply 实现一个Function.bind 实现一个继承 实现一个JS函数柯里化 手写一个Promise(中高级必考。"
sidebarWeight: 24
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/a-吊打面试官/考点难点/「中高级前端面试」JavaScript手写代码无敌秘籍.md"
---
::: v-pre

# 「中高级前端面试」JavaScript手写代码无敌秘籍

> 本节目标：理解“「中高级前端面试」JavaScript手写代码无敌秘籍”的核心思路，并能把它用于实际开发或面试表达。
2019-09-29阅读 1970
**手写路径导航**
...
与 `Function` 都有着动态编译js代码的作用，但是在实际的编程中并不推荐使用。
...
```
Function.prototype.apply2 = function(context = window) {    context.fn = this    let result;    //
```
...
```
Function.prototype.bind2 = function(content) {    if(typeof this != "function") {        throw Error("not a function")    }    //
```

若没问参数类型则从这开始写

```
    let fn = this;    let args = [...arguments].slice(1);
...
```
var promise = new Promise((resolve,reject) => {    if (
```

操作成功

```
) {        resolve(value)    } else {        reject(error)    }})promise.then(function (value) {    // success},function (value) {    // failure})
```
 **8.2 面试够用版**
来源：实现一个完美符合Promise/A+规范的Promise

```
function myPromise(constructor){    let self=this;    self.status="pending" //
```
...
定义状态为`resolved`的时候的状态

```
    self.reason=undefined;//
```

定义状态为`rejected`的时候的状态
...
```
       if(self.status==="pending"){          self.value=value;          self.status="resolved";       }    }    function reject(reason){        //
```
...
function Promise(excutor) {    let that = this; //
```
...
```
    that.value = undefined; // fulfilled
```

状态时 返回的信息

```
    that.reason = undefined; // rejected
```
...
存储`fulfilled`状态对应的`onFulfilled`函数

```
    that.onRejectedCallbacks = []; //
```

存储`rejected`状态对应的`onRejected`函数
...
只能由`pending`状态 `=\> fulfilled`状态 `(`避免调用多次

```
resolve reject)                that.status = FULFILLED;                that.value = value;                that.onFulfilledCallbacks.forEach(cb => cb(that.value));            }        });    }    function reject(reason) { // reason
```
...
只能由`pending`状态 `=\> rejected`状态 `(`避免调用多次

```
resolve reject)                that.status = REJECTED;                that.reason = reason;                that.onRejectedCallbacks.forEach(cb => cb(that.reason));            }        });    }
//
```
...
```
    // new Promise((resolve, reject) => {    //     throw new Error('error in excutor')    // })    try {        excutor(resolve, reject);    } catch (e) {        reject(e);    }}
Promise.prototype.then = function(onFulfilled, onRejected) {    const that = this;    let newPromise;    //
```

处理参数默认值 保证参数后续能够继续执行

```
    onFulfilled =        typeof onFulfilled === "function" ? onFulfilled : value => value;    onRejected =        typeof onRejected === "function" ? onRejected : reason => {            throw reason;        };    if (that.status === FULFILLED) { //
```

成功态

```
        return newPromise = new Promise((resolve, reject) => {            setTimeout(() => {                try{                    let x = onFulfilled(that.value);                    resolvePromise(newPromise, x, resolve, reject); //
```

新的`promise resolve` 上一个`onFulfilled`的返回值

```
                } catch(e) {                    reject(e); //
```

捕获前面`onFulfilled`中抛出的异常
...
```
        return newPromise = new Promise((resolve, reject) => {            setTimeout(() => {                try {                    let x = onRejected(that.reason);                    resolvePromise(newPromise, x, resolve, reject);                } catch(e) {                    reject(e);                }            });        });    }
if (that.status === PENDING) { //
```
...
```
        return newPromise = new Promise((resolve, reject) => {            that.onFulfilledCallbacks.push((value) => {                try {                    let x = onFulfilled(value);                    resolvePromise(newPromise, x, resolve, reject);                } catch(e) {                    reject(e);                }            });            that.onRejectedCallbacks.push((reason) => {                try {                    let x = onRejected(reason);                    resolvePromise(newPromise, x, resolve, reject);                } catch(e) {                    reject(e);                }            });        });    }};
```
 emmm，我还是乖乖地写回进阶版吧。
...
```
        var result = obj.constructor == Array ? [] : {};        for(let i in obj){            result[i] = typeof obj[i] == "object" ? deepCopy(obj[i]) : obj[i];        }    }else {        //
```

简单数据类型 直接 `==` 赋值

```
        var result = obj;    }    return result;}
```
...
- 为何你始终理解不了JavaScript作用域链？ \> 来自 \<[https://cloud.tencent.com/developer/article/1515933](https://cloud.tencent.com/developer/article/1515933)\>

\> 来自 \<[https://cloud.tencent.com/developer/article/1515933](https://cloud.tencent.com/developer/article/1515933)\>
