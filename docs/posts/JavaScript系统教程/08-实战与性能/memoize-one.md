...
tags:
  ...
  - "OneNote"
  - "实战与性能"
description: "编写更快的 React 代码（一）： memoize one 简介 引言 不同类型业务要求的性能标准各不相同。如果对一个 ToB 的后台管理系统要求首屏速度以及 SEO ，显然不合理也没必要。 第一要考虑的不是如何去优化，而是值不值得去优化， React 性能已经足够优秀，毕竟“。"
sidebarWeight: 20
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/b-原生js/经典方法/memoize-one.md"
---
::: v-pre

# memoize-one

> 本节目标：理解“memoize-one”的核心思路，并能把它用于实际开发或面试表达。
...
2. 减少 `rerender` 次数 （`immutable data`、`shouldComponentUpdate`、`PureComponent`）
3. 减轻 `rerender` 复杂度 （`memoize-one`）
...
render() {    const filteredList = this.props.list.filter(item =>      item.text.includes(this.state.filterText)    );
return (      <Fragment>        <input onChange={this.handleChange} value={this.state.filterText} />        <ul>          {filteredList.map(item => (            <li key={item.id}>{item.text}</li>          ))}        </ul>      </Fragment>    );  }}
```
...
5. `state(filterText)/props(list)`未发生变化时，不进行 `render(`引言`-`性能优化第 `1` 点`)`， 此处暂不讨论
6. `state(filterText)/props(list)`未发生变化时，进行 `render`，复用上一次计算结果
...
==只有在==`list`==或==`filterText`==改变的时候才会重新调用真正的==`filter`==方法（==`memoize`==入参）==
...
==在上一次==`render`==后，如果参数没有发生改变，==`` `memoize-one` ``==会重复使用上一次的返回结果==
...
```
    lastResult = resultFn.apply(this, newArgs);    calledOnce = true;    //
```
...
```
const createFab = () => {  const cache = [0, 1, 1];  return n => {    if (typeof cache[n] !== "undefined") {      return cache[n];    }    for (let i = 3; i <= n; i++) {      if (typeof cache[i] !== "undefined") continue;      cache[i] = cache[i - 1] + cache[i - 2];    }    return cache[n];  };};
const fab = createFab();
```
 总结
