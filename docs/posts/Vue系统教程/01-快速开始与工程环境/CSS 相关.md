...
tags:
  ...
  - "OneNote"
  - "快速开始与工程环境"
description: "Vue CLI 项目天生支持 、 和包含 、 、 在内的预处理器。 引用静态资源 所有编译后的 CSS 都会通过 来解析其中的 url() 引用，并将这些引用作为模块请求来处理。这意味着你可以根据本地的文件结构用相对路径来引用静态资源。另外要注意的是如果你想要引用一个 npm 依。"
sidebarWeight: 2
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/vue-cli/CSS 相关.md"
---
::: v-pre

# CSS 相关

> 本节目标：理解“CSS 相关”的核心思路，并能把它用于实际开发或面试表达。
`Vue CLI` ==项目天生支持==
...
```
 ...
 ==来解析其中的== `url()` ==引用，并将这些引用作为模块请求来处理。这意味着你可以根据本地的文件结构用相对路径来引用静态资源。另外要注意的是如果你想要引用一个== `npm` ==依赖中的文件，或是想要用== `webpack alias`==，则需要在路径前加上== `~` ==的前缀来避免歧义。更多细节请参考====处理静态资源====。==
...
# Stylus
npminstall-D stylus-loader stylus
```
 ==然后你就可以导入相应的文件类型，或在== `*.vue` ==文件中这样来使用：==

```
<stylelang="scss">  $color:red;
</style>
```
...
```
 ...
 ==支持的配置源来配置== `PostCSS`==。也可以通过== `vue.config.js` ==中的== `css.loaderOptions.postcss` ==配置==
...
#CSS Modules
```
 ==你可以通过== `\<style module\>` ==以开箱即用的方式====在== `*.vue` ==文件中使用== `CSS Modules`==。==
...
```
importsassStyles from'./foo.module.scss'
```
