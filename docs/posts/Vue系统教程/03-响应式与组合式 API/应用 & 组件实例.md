...
tags:
  ...
  - "OneNote"
  - "响应式与组合式 API"
description: "创建一个应用实例 每个 Vue 应用都是通过用 createApp 函数创建一个新的应用实例开始的： Const app Vue.createApp(\\{/ 选项 /\\}) 该应用实例是用来在应用中注册“全局”组件的。我们会在后面的指南中详细讨论，简单的例子： 应用实例暴露的大多。"
sidebarWeight: 96
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/vue3/应用 & 组件实例.md"
---
::: v-pre

# 应用 & 组件实例

> 本节目标：理解“应用 & 组件实例”的核心思路，并能把它用于实际开发或面试表达。
...
==应用实例暴露的大多数方法都会返回该同一实例，====所以可====链式====调用====：==
...
==与大多数应用====实例的====方法不同的是，====mount== ==不返回应用本身。相反，它返回的是根组件实例。==
...
==我们会在稍后的====组件基础====章节具体展开。不过现在，你只需要明白根组件与其他组件没什么不同，配置选项是一样的，所对应的组件实例行为也是一样的。==
...
\> 来自 \<[https://v3.cn.vuejs.org/guide/instance.html#%E7%94%9F%E5%91%BD%E5%91%A8%E6%9C%9F%E5%9B%BE%E7%A4%BA](https://v3.cn.vuejs.org/guide/instance.html#%E7%94%9F%E5%91%BD%E5%91%A8%E6%9C%9F%E5%9B%BE%E7%A4%BA)\>
