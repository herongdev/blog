...
tags:
  ...
  - "OneNote"
  - "原理与手写实现"
description: "VUEshop 中国免费商城系统 搜豹商城系统 免费50小时 Vue 视频教程 立即查看 \\ 现在是时候深入一下了！Vue 最独特的特性之一，是其非侵入性的响应式系统。数据模型仅仅是普通的 JavaScript 对象。而当你修改它们时，视图会进行更新。这使得状态管理非常简单直接。"
sidebarWeight: 41
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/原理/深入响应式原理/深入响应式原理.md"
---
::: v-pre

# 深入响应式原理

> 本节目标：理解“深入响应式原理”的核心思路，并能把它用于实际开发或面试表达。

> 说明：原 OneNote 中有图片引用，但图片未包含在导出目录中；本页保留了可用的文字与代码内容。
[VUEshop 中国免费商城系统 - 搜豹商城系统 - 免费50小时 Vue 视频教程 立即查看 \>](http://www.vueshop.com.cn/index.php?http_referer=vuejs)
...
```
 \> 来自 \<[https://cn.vuejs.org/v2/guide/reactivity.html](https://cn.vuejs.org/v2/guide/reactivity.html)\>
...
这些 `getter/setter` 对用户来说是不可见的，但是在内部它们让 `Vue` 能够追踪依赖，在 `property` 被访问和修改时通知变更。这里需要注意的是不同浏览器在控制台打印数据对象时对 `getter/setter` 的格式化并不同，所以建议安装
