...
tags:
  ...
  - "OneNote"
  - "模板、组件与交互"
description: "是可复用的 Vue 实例。可进行任意次数的复用，每用一次组件，就会有一个它的新实例被创建。 data 必须是一个函数： 组件的 data 选项必须是一个函数，因此每个实例可以维护一份被返回对象的独立的拷贝。否则所有组件实例会共用同一个数据对象，即指向同一个内存地址： 组件的组织。"
sidebarWeight: 104
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/组件/组件基础/组件基础.md"
---
::: v-pre
...
是可复用的 Vue 实例。可进行任意次数的复用，每用一次组件，就会有一个它的新实例被创建。

```
名字：通常有一个名字，如上例的 <button-counter>；
```

- ```
    选项：组件与 new Vue 接收相同的选项，例如：
    ```

    ```
    data、computed、watch、methods等；
    ```
    ...
    **data** **必须是一个函数：**组件的 data 选项必须是一个函数，因此每个实例可以维护一份被返回对象的独立的拷贝。否则所有组件实例会共用同一个数据对象，即指向同一个内存地址：
...
```
**通过** **Prop** **向子组件传递数据**
Prop 是在组件上注册的一些自定义 attribute。当一个值传递给一个 prop attribute 的时候，它就变成了那个组件实例的一个 property。
例如：为了给博文组件传递一个标题，我们可以用一个 props选项将其包含在该组件可接受的 prop 列表中：
...
如上所示，你会发现我们可以使用 v-bind 来动态传递 prop。这在你一开始不清楚要渲染的具体内容，比如[从一个](https://codesandbox.io/s/github/vuejs/vuejs.org/tree/master/src/v2/examples/vue-20-component-blog-post-example) API 获取博文列表的时候，是非常有用的。
```
...
同时子组件可以通过调用内建的 [$emit](https://cn.vuejs.org/v2/api/#vm-emit) 方法并传入事件名称来触发一个事件：
