...
tags:
  ...
  - "OneNote"
  - "补充主题"
description: "接下来你需要熟悉的是如何在 createElement 函数中使用模板中的那些功能。这里是 createElement 接受的参数： 一个 HTML 标签名、组件选项对象，或者 了上述任何一种的一个 async 函数。必填项。 一个与模板中 attribute 对应的数据对象。可。"
sidebarWeight: 7
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/可复用和组合/渲染函数 & JSX/createElement 参数.md"
---
::: v-pre

# createElement 参数

> 本节目标：理解“createElement 参数”的核心思路，并能把它用于实际开发或面试表达。
...
```
render: function (createElement) {  var myParagraphVNode = createElement('p', 'hi')  return createElement('div', [
```
...
```
 ...
 `.13`|`if (event.keyCode !== 13) return` `(`对于别的按键修饰符来说，可将 `13` 改为[另一个按键码](http://keycode.info/)`)`|
|修饰键：
...
```
 ...
 `.meta`|`if (!event.ctrlKey) return` `(`将 `ctrlKey` 分别修改为 `altKey`、`shiftKey` 或者
