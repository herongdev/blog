---
title: "Pinia 请求 store 请求 页面中请求 401 权限"
date: 2026-08-11
categories:
  - "React 系统教程"
tags:
  ...
  - "OneNote"
  - "原理与手写实现"
description: "核心结论 ： \\ 把“请求放在 Store”与“何时触发请求”解耦 。 \\ 数据仍由 Pinia Store 管，但所有“首次加载/刷新”的触发统一由 受保护布局(ProtectedLayout) 或 集中启动钩子 在“已登录且当前路由需要鉴权”时一次性触发。 \\ 请求层（Axi。"
sidebarWeight: 42
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/实现/Pinia 请求 store 请求 页面中请求 401 权限.md"
---
::: v-pre

# Pinia 请求 store 请求 页面中请求 401 权限

> 本节目标：理解“Pinia 请求 store 请求 页面中请求 401 权限”的核心思路，并能把它用于实际开发或面试表达。
****核心结论****：
...
下面给你****最佳做法****与****只需改动的代码片段****（按你的要求，****不贴整文件****；复杂逻辑在上一行有注释）：
---
...
export function useAppBootstrap() {
  ...
  watch(
    [() => auth.isAuthenticated, () => router.currentRoute.value.meta.requiresAuth],
    async ([ok, need]) => {
      if (ok && need) {
...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function refreshToken() {
    ...
    const { data } = await axios.post('/auth/refresh', { refresh_token: refresh.value })
