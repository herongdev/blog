...
tags:
  ...
  - "OneNote"
  - "原理与手写实现"
description: "把虚拟 DOM 变成真实 DOM 插入到容器内部 虚拟 容器 把虚拟 DOM 转成真实 真实 如果这个元素是一个文本的话 如果类型是一个函数的话 说明它是一个类组件 处理属性 说明这是一个 React 元素 让虚拟 DOM 的 dom 属性指向这个虚拟 DOM 对应的真实 将 r。"
sidebarWeight: 76
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/实现/类组件的更新 /src-react-dom.js.md"
---
::: v-pre
...
将`renderVdom`放在了实例之上，不像之前放在组件自身`vdom`上；
