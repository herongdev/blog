...
tags:
  ...
  - "OneNote"
  - "核心概念与组件"
description: "Profiler 测量渲染一个 React 应用多久渲染一次以及渲染一次的“代价”。 它的目的是识别出应用中渲染较慢的部分，或是可以使用类似 memoization 优化的部分，并从相关优化中获益。 注意： Profiling 增加了额外的开支，所以它在生产构建中会被禁用。 为了。"
sidebarWeight: 4
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/新特性/Profiler API.md"
---
::: v-pre

# Profiler API

> 本节目标：理解“Profiler API”的核心思路，并能把它用于实际开发或面试表达。
Profiler 测量渲染一个 React 应用多久渲染一次以及渲染一次的“代价”。
它的目的是识别出应用中渲染较慢的部分，或是可以使用[类似](https://react.docschina.org/docs/hooks-faq.html#how-to-memoize-calculations) memoization 优化的部分，并从相关优化中获益。

**注意：**
Profiling 增加了额外的开支，所以它在[生产构建](https://react.docschina.org/docs/optimizing-performance.html#use-the-production-build)中会被禁用。
...
```
interactions: Set - 当更新被制定时，[“](https://fb.me/react-interaction-tracing)interactions” 的集合会被追踪。（例如当 render 或者 setState 被调用时）。
```
