...
`\<a href="account/findAccount?accountId=10"\>`
...
\<input type="text" name="address.cityName" \>\<br/\>
...
\<input type="text" name="accounts[1].name" \>\<br/\>
 账户
...
\<input type="text" name="accountMap['one'].name" \>\<br/\>
 账户
...
\<input type="text" name="accountMap['two'].name" \>\<br/\>
 账户
...
表示路径，`mapping` 表示文件，`**`表示该目录下的文件以及子目录的文件
...
对 `GET` 和 `POST` 请求处理方式是不同的，`GET` 请求的编码问题，要改 `tomcat` 的 `server.xml`
配置文件，如下：
...
- --\>
- \<a href="account/deleteAccount?date=2018-01-01"\>
...
![HTTP Status 400 Bad Request Status Report The serv...](Exported%20image%2020260702233400-0.png)
...
public interface Converter\<S, T\> {//S:
表示接受的类型，`T`：表示目标类型
...
配置类型转换器的机制是，将自定义的转换器注册到类型转换服务中去。￼`\<!--` 配置类型转换器工厂
...
/**
*
测试访问 `testServletAPI`传智播客——专注于 `Java`、`.Net` 和`Php`、网页平面设计工程师的培训
* @return
*/
...
![properties Servers Console Data Source Explorer Sn...](Exported%20image%2020260702233402-1.png)
