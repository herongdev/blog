...
 中，实例化 `Bean` 有三种方式，分别是构造器实例化、静态工厂方式实例化和实例工厂方式实例化。本节将针对这三种方式分别进行讲解。
**构造器实例化**
...
注意：为了方便读者的学习，本节中的所有配置文件和 

`Java`

 文件都根据知识点放置在同一个包中。在实际开发中，为了方便管理和维护，建议将这些文件根据类别放置在不同目录中。
**静态工厂方式实例化**
...
8. `\<!-- factory-bean`属性指定一个实例工厂，`factory-method`属性确定使用工厂中的哪个方法 `--\>`
9. ```
