...
是基础类型的 `IoC` 容器，它由 `org.springframework.beans.facytory.BeanFactory` 接口定义，并提供了完整的 `IoC` 服务支持。简单来说，`BeanFactory` 就是一个管理 `Bean` 的工厂，它主要负责初始化各种`Bean`，并调用它们的生命周期方法。
...
ApplicationContext
ApplicationContext
是 `BeanFactory` 的子接口，也被称为应用上下文。该接口的全路径为 `org.springframework.context.ApplicationContext`，它不仅提供了 `BeanFactory` 的所有功能，还添加了对 `i18n`（国际化）、资源访问、事件传播等方面的良好支持。
...
将加载`spring`目录下的`applicationContext.xml`文件
