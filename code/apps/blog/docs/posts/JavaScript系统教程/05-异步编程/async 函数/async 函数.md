...
tags:
  ...
  - "OneNote"
  - "异步编程"
description: "内置执行器：Generator 函数的执行必须靠执行器，所以才有了co模块，而async函数自带执行器。所以，async函数的执行，只要一行： 语义更好：async和await，比起星号和yield，语义更清楚了。async表示函数里有异步操作，await表示紧跟在后面的表达式需。"
sidebarWeight: 50
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/b-原生js/11-异步编程/async 函数/async 函数.md"
---
::: v-pre

# async 函数

> 本节目标：理解“async 函数”的核心思路，并能把它用于实际开发或面试表达。
```
...
内置执行器：Generator 函数的执行必须靠执行器，所以才有了co模块，而async函数自带执行器。所以，async函数的执行，只要一行：

```
asyncReadFile();
```

语义更好：async和await，比起星号和yield，语义更清楚了。async表示函数里有异步操作，await表示紧跟在后面的表达式需要等待结果。
