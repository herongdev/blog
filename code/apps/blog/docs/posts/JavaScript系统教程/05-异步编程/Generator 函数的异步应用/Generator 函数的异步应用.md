...
tags:
  ...
  - "OneNote"
  - "异步编程"
description: "Promise 对象就是为了解决回调地狱而提出的。它不是新的语法功能，而是一种新的写法，允许将回调函数的嵌套，改成链式调用。 Generator 函数 协程 传统的编程语言，早有异步编程的解决方案（其实是多任务的解决方案）。其中有一种叫做\"协程\"（coroutine），意思是多个。"
sidebarWeight: 9
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/b-原生js/11-异步编程/Generator 函数的异步应用/Generator 函数的异步应用.md"
---
::: v-pre

# Generator 函数的异步应用

> 本节目标：理解“Generator 函数的异步应用”的核心思路，并能把它用于实际开发或面试表达。
```
**传统方法**
...
function* gen(x){  var y = yield x + 2;  return y;}
var g = gen(1);g.next() // { value: 3, done: false }g.next(2) // { value: 2, done: true }
...
var g = gen(1);g.next();g.throw('出错了');// 出错了
...
function* gen(){  var url = '[https://api.github.com/users/github](https://api.github.com/users/github)';  var result = yield fetch(url);  console.log(result.bio);}
...
它的 Thunk 函数替换的不是表达式，而是多参数函数，将其替换成一个只接受回调函数作为参数的单参数函数。
// 正常版本的readFile（多参数版本）fs.readFile(fileName, callback);
// Thunk版本的readFile（单参数版本）var Thunk = function (fileName) {  return function (callback) {    return fs.readFile(fileName, callback);  };};
...
下面是另一个完整的例子。
function f(a, cb) {  cb(a);}const ft = Thunk(f);
ft(1)(console.log) // 1
```
...
var read = thunkify(fs.readFile);read('package.json')(function(err, str){  // ...});
...
var ft = thunkify(f);var print = console.log.bind(console);ft(1, 2)(print);// 3
...
var fs = require('fs');var thunkify = require('thunkify');var readFileThunk = thunkify(fs.readFile);
var gen = function* (){  var r1 = yield readFileThunk('/etc/fstab');  console.log(r1.toString());  var r2 = yield readFileThunk('/etc/shells');  console.log(r2.toString());};
上面代码中，yield命令用于将程序的执行权移出 Generator 函数，那么就需要一种方法，将执行权再交还给 Generator 函数。
这种方法就是 Thunk 函数，因为它可以在回调函数里，将执行权交还给 Generator 函数。为了便于理解，我们先看如何手动执行上面这个 Generator 函数。
var g = gen();
var r1 = g.next();r1.value(function (err, data) {  if (err) throw err;  var r2 = g.next(data);  r2.value(function (err, data) {    if (err) throw err;    g.next(data);  });});
...
run(g);
```
 上面代码的run函数，就是一个 Generator 函数的自动执行器。内部的next函数就是 Thunk 的回调函数。next函数先将指针移到 Generator 函数的下一步（gen.next方法），然后判断 Generator 函数是否结束（result.done属性），如果没结束，就将next函数再传入 Thunk 函（result.value属性），否则就直接退出。

yield后必然是异步操作，当异步操作有了结果时，会通过回调函数把结果提供出来。

thunk化的函数特点就是先接受除回调函数以外的所有参数，然后返回一个函数，这个函数接受回调函数，并且，在内部，当异步操作有结果时，会调用回调函数把结果或错误传递出来。

我们在yield后调用thunk化的函数，接受了除回调以外的所有参数，所以，他返回了一个接受回调作为参数的函数。

执行general 函数时，我们先执行得到遍历器对象，接下来调用遍历器对象的next方法，也就是会执行yield方法后的thunk 函数，这里给这个函数传入了除回调函数外的所有参数，得到一个对象，它的value值就是thunk 函数的执行结果，他是一个函数，他接受回调函数作为参数。

这时，我们判断done 值，看general 函数是否有其他yield表达式，如果没有表示已经完成，则退出函数。

如果还有其他yield表达式，则done 值为false，我们就给value值，也就是thunk函数的返回的那个只接受回调函数的函数传入一个回调函数。这样的话，thunk函数会开始执行，并在出现结果时执行我们传入的回调函数。

这个回调函数的第一个参数是err，第二个参数是回调函数的异步操作的结果，函数内部，由于general 函数未完成，我们要继续调用遍历器对象的next方法，并且给next传入异步操作的结果。

接下啦我们判断next的结果对象的done属性，看异步操作是否完成，如果完成就结束函数，如果没有完成，我们又把同样的回调函数传给next的value，即传给了另一个接受回调函数的thunk 函数的返回函数。

总之，执行yield后的函数，如果所有异步没执行完，则给这个yield后的执行结果传入一个回调函数。

当提供回调函数后，yield后的异步操作有结果了会给把结果传递给回调函数。

回调函数中，调用next方法，并传递异步结果给他。接下来看是否异步操作多结束了，没有的话，继续把同样的回调函数传递给下一个yield后的表达式执行结果，这个表达式的执行结果一般是个接受回调函数的函数。这个函数内部执行异步操作，有结果了，调用传入的回调函数。
...
_Thunk_ _函数__执行后会返回一个函数，这个函数接受一个回调函数作为参数。_
_传入的回调函数做以下事情：_

```
_调用__next__方法__，并传入异步操作的结果。_
```

_判断__next__方法返回的对象的__done__属性__，完成则结束，没完成就执行对象的__value__值__，也就是__thunk__函数执行后返回的接受回调函数的函数。_
...
**co** **模块**
**基本用法**
[co](https://github.com/tj/co) 模块是著名程序员 TJ Holowaychuk 于 2013 年 6 月发布的一个小工具，用于 Generator 函数的自动执行。
...
function run(gen){  var g = gen();
function next(data){    var result = g.next(data);    if (result.done) return result.value;    result.value.then(function(data){      next(data);    });  }
next();}
run(gen);
...
function co(gen) {  var ctx = this;
return new Promise(function(resolve, reject) {    if (typeof gen === 'function') gen = gen.call(ctx);    if (!gen || typeof gen.next !== 'function') return resolve(gen);
onFulfilled();    function onFulfilled(res) {      var ret;      try {        ret = gen.next(res);      } catch (e) {        return reject(e);      }      next(ret);    }  });}
...
// 数组的写法co(function* () {  var res = yield [    Promise.resolve(1),    Promise.resolve(2)  ];  console.log(res);}).catch(onerror);
// 对象的写法co(function* () {  var res = yield {    1: Promise.resolve(1),    2: Promise.resolve(2),  };  console.log(res);}).catch(onerror);
下面是另一个例子。
co(function* () {  var values = [n1, n2, n3];  yield values.map(somethingAsync);});
function* somethingAsync(x) {  // do something async  return y}
