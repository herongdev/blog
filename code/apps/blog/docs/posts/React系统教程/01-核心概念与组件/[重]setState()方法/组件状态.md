...
tags:
  ...
  - "OneNote"
  - "核心概念与组件"
description: "setState 实际做了什么？ setState() 会对一个组件的 state 对象安排一次更新。当 state 改变了，该组件就会重新渲染。 我应该如何更新那些依赖于当前的 state 的 state 呢？ 给 setState 传递一个函数，而不是一个对象，就可以确保每次。"
sidebarWeight: 88
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/状态改变/[重]setState()方法/组件状态.md"
---
::: v-pre

# 组件状态

> 本节目标：理解“组件状态”的核心思路，并能把它用于实际开发或面试表达。
**setState** **实际做了什么？**
...
```
props 是传递_给_组件的（类似于函数的形参）；
```

```
state 是在组件_内_被组件自己管理的（类似于在一个函数内声明的变量）。
```
...
handleSomething() {  // 假设 `this.state.count` 从 0 开始。  this.incrementCount();  this.incrementCount();  this.incrementCount();
// 如果你现在在这里读取 `this.state.count`，它还是会为 0。  // 但是，当 React 重新渲染该组件时，它会变为 3。}
[学习更多有关](https://react.docschina.org/docs/react-component.html#setstate) setState 的内容
**setState** **什么时候是异步的？**
...
```
这个 [GitHub](https://github.com/facebook/react/issues/11527#issuecomment-360199710) 评论 深入了该特殊示例。
```
...
```
 <https://react.docschina.org/docs/faq-state.html>
```
