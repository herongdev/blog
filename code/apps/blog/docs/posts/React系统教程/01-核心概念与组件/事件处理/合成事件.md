...
tags:
  ...
  - "OneNote"
  - "核心概念与组件"
description: "此参考指南记录了构成 React 事件系统一部分的 SyntheticEvent 包装器。请参考有关事件处理的指南来了解更多。 概览 SyntheticEvent 实例将被传递给你的事件处理函数，它是浏览器的原生事件的跨浏览器包装器。除兼容所有浏览器外，它还拥有和浏览器原生事件相。"
sidebarWeight: 56
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/概念/事件处理/合成事件.md"
---
::: v-pre

# 合成事件

> 本节目标：理解“合成事件”的核心思路，并能把它用于实际开发或面试表达。
...
```
**注意：**
从 v17 开始，e.persist() 将不再生效，因为 SyntheticEvent 不再放入[事件池](https://react.docschina.org/docs/legacy-event-pooling.html)中。
注意：
```
...
function Example() {  return (    <input      onFocus={(e) => {        console.log('Focused on input');      }}      placeholder="onFocus is triggered when you click this input."    />  )}
**onBlur**
