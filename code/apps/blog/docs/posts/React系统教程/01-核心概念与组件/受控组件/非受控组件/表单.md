...
feed: false
source: onenote
sourceNote: "OneNote/f-vue/概念/受控组件/非受控组件/表单.md"
---
::: v-pre

# 表单

> 本节目标：理解“表单”的核心思路，并能把它用于实际开发或面试表达。
```
...
handleChange(event) {    this.setState({value: event.target.value});  }  handleSubmit(event) {    alert('你喜欢的风味是: ' + this.state.value);    event.preventDefault();  }
...
handleInputChange(event) {    const target = event.target;    const value = target.name === 'isGoing' ? target.checked : target.value;    const name = target.name;    this.setState({      [name]: value    });  }
...
另外，由于 setState() 自动[将部分](https://react.docschina.org/docs/state-and-lifecycle.html#state-updates-are-merged) state 合并到当前 state, 只需调用它更改部分 state 即可。
**受控输入空值**
