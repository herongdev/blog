...
tags:
  ...
  - "OneNote"
  - "原理与手写实现"
description: "\\ 来自 从源码可以看出虽然创建的时候都是通过 config 传入的，但是 key 和 ref 不会跟其他 config 中的变量一起被处理，而是单独作为变量出现在 ReactElement 上。 在最后创建 ReactElement 我们看到了这么一个变量 $$typeof。"
sidebarWeight: 4
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/原理 2/ReactElement/ReactElement.md"
---
::: v-pre

# ReactElement

> 本节目标：理解“ReactElement”的核心思路，并能把它用于实际开发或面试表达。
```
==export function== ==createElement====(====type, config, children====)== =={======  ==//== ==处理参数==
...
```
==const== ==ReactElement === ==function====(====type, key, ref, self, source, owner, props====)== =={======  ==const== ==element = {======    ==// This tag allows us to uniquely identify this as a React Element======    ==$$====typeof====: REACT_ELEMENT_TYPE,======
