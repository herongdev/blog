...
tags:
  ...
  - "OneNote"
  - "前端知识复习"
description: "但有时候由于编写闭包的方式不同，这一点可能不会那么明显。下面来看一个例子。 为什么匿名函数没有取得其包含作用域(或外部作用域)的this对象呢？ 前面曾经提到过，每个函数在被调用时都会自动取得两个特殊变最：this和arguments。内部函数在搜索这两个变量时，只会搜索到其活动。"
sidebarWeight: 19
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/a-吊打面试官/考点难点/(20m)重this对象.md"
---
::: v-pre
...
前面曾经提到过，每个函数在被调用时都会自动取得两个特殊变最：this和arguments。内部函数在搜索这两个变量时，只会搜索到其活动对象为止，因此永远不可能直接访问外部函数中的这两个变量(这一点通过图7-2可以看得更清楚)。
