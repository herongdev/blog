...
tags:
  ...
  - "OneNote"
  - "路由与状态管理"
description: "在创建路由器实例时，history 配置允许我们在不同的历史模式中进行选择。 Hash 模式 hash 模式是用 createWebHashHistory() 创建的： import \\{ createRouter, createWebHashHistory \\} from 'v。"
sidebarWeight: 7
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/vueRouter/HTML5 History 模式.md"
---
::: v-pre

# HTML5 History 模式

> 本节目标：理解“HTML5 History 模式”的核心思路，并能把它用于实际开发或面试表达。
...
\})
当使用这种历史模式时，URL 会看起来很 "正常"，例如 [https://example.com/user/id](https://example.com/user/id。漂亮)。漂亮!

不过，问题来了。由于我们的应用是一个单页的客户端应用，如果没有适当的服务器配置，用户在浏览器中直接访问 [https://example.com/user/id](https://example.com/user/id，就会得到一个)，就会得到一个 404 错误。这就尴尬了。
