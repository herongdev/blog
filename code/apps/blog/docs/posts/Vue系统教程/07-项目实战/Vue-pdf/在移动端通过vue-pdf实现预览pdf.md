...
tags:
  ...
  - "OneNote"
  - "项目实战"
description: "应用场景： 比赛报表pdf文件需要在线预览跟下载，发现直接使用window.open(url, ' blank')，在pc端是可以实现预览，但是在移动端是直接通过打开浏览器进行下载再预览，经查资料发现可以使用vue pdf组件实现预览！！！ 参考： https://www.cnb。"
sidebarWeight: 45
lastUpdated: false
feed: false
source: onenote
sourceNote: "OneNote/f-vue/实战/Vue-pdf/在移动端通过vue-pdf实现预览pdf.md"
---
::: v-pre

# 在移动端通过vue-pdf实现预览pdf

> 本节目标：理解“在移动端通过vue-pdf实现预览pdf”的核心思路，并能把它用于实际开发或面试表达。
