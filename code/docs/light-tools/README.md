...
当前工具箱是独立的 Next.js 应用，代码放在站点 monorepo 的 `code/apps/light-tools/web/` 目录里。博客的 VitePress 导航已经加入“工具箱”菜单，本地默认指向：
...
1. 单独部署 `code/apps/light-tools/web`。
...
docker compose -f infra/light-tools/docker-compose.prod.yml up -d --build
```

Nginx 需要在博客 `server { ... }` 中包含或复制：

```text
code/infra/nginx/light-tools-location.conf
```
...
```text
code/docs/light-tools/wechat-pay-integration-plan.md
```
...
生产 compose 会把 `code/secrets/` 目录只读挂载到容器内 `/run/secrets/light-tools`。`secrets/` 已加入 `.gitignore` 和 `.dockerignore`，不要把私钥提交进 Git。
