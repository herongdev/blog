...
> 当前实际代码已纳入站点级 monorepo：前端位于 `code/apps/light-tools/web`，Worker 位于 `code/apps/light-tools/worker`，共享包位于 `code/packages/light-tools/`。本文后文保留的 `apps/web`、`packages/shared` 等短路径是 LightTools 子项目内的逻辑路径。
