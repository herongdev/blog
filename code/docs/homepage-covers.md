...
首页现已改用 `FieldPreview.vue` 内的原生 SVG 领域关系图，旧版生成图片不再加载，但保留以便回退。以下生成记录属于历史版本。

六个构图主题为编排、映射、尺度、闭环、实时传播和知识组织。统一线宽与留白，使用各领域的主题色及独立明暗色彩变量；不画假产品界面，不以生成视觉冒充项目成果。此处 SVG 是内容图解，不是替代成熟图标库的操作图标。
